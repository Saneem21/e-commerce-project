...
if (isset($_POST['re_password']))
  ...
  $password_query = mysqli_query($con,"select * from admin_register where admin_mail='$email'");
  $password_row = mysqli_fetch_array($password_query);
  $database_password = $password_row['admin_pwd'];
  if ($database_password == $op)
    {
    if ($new_pass == $re_pass)
      {
        $pass = md5($re_pass);
      $update_pwd = mysqli_query($con,"UPDATE admin_register set admin_pwd='$pass' where admin_name='$name'");
