...
<td><input type="text"  name="aname" value="" maxlength = "16" class="text-bars" placeholder="Username" required></td>
 ...
 <td><b>:</b></td>
 <td><input type="email"  name="amail" value="" placeholder="Email Id"required></td>
 </tr>
 ...
 <td><b>:</b></td>
 <td><input type="password" value="FakePSW" placeholder="minumun 8 character" name ="apwd" id="myInput" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" required>
...
<h5>PASSWORD SHOULD HAVE  ATLEAST 1CAPTITAL LETTER ,1 SPECIAL CHARACTER,AND 1 NUMBER </h5>
 ...
 if(isset($_REQUEST['signup'])){
   ...
   $db=mysqli_select_db($con,"ecom")or die(mysql_error());
     ...
     $query="insert into admin_register values('".$_REQUEST["id"]."','".$_REQUEST["aname"]."','".$_REQUEST["amail"]."','".$_REQUEST["apwd"]."')";
 if(mysqli_query($con,$query))
 {
    echo header("Location:sidenav.php?admin_name=".$_REQUEST['aname']);
 }
 else
     echo "Invalid";
   }
